...
current_roll = 0.0   # 当前roll角度，移动时保持

def send_cmd(mode, gait_id=0, vx=0.0, vy=0.0, vyaw=0.0, duration=500, pitch=None, roll=None):
    global life_count, current_pitch, current_roll
    # pitch=None 时使用当前保持的pitch值
    p = current_pitch if pitch is None else pitch
    r = current_roll if roll is None else roll
    with lock:
...
def keep_alive(stop_event):
    while not stop_event.is_set():
        with lock:
            msg.rpy_des = [current_roll, current_pitch, 0.0]
...
def main():
    ...
    try:
        while True:
            ...
            elif key == 'o':  # 抬头（pitch后仰，负值为后仰）
                current_pitch = -0.20
                send_cmd(MODE_LOCOMOTION, gait_id=GAIT_TROT, duration=500, pitch=current_pitch)
                print("\r>> 抬头 pitch=-0.20（移动时保持）   ", end='', flush=True)

            elif key == 'k': # 左滚（roll左倾，正值为左倾）
                current_roll = 0.12
                send_cmd(MODE_LOCOMOTION, gait_id=GAIT_TROT, duration=500, roll=current_roll)
                print("\r>> 左滚 roll=0.12（移动时保持）     ", end='', flush=True)
            
            elif key == 'l': # 右滚（roll右倾，负值为右倾）
                current_roll = -0.12
                send_cmd(MODE_LOCOMOTION, gait_id=GAIT_TROT, duration=500, roll=current_roll)
                print("\r>> 右滚 roll=-0.12（移动时保持）     ", end='', flush=True)

            elif key == 'u':  # 恢复正常姿态
                current_pitch = 0.0
                current_roll = 0.0
                send_cmd(MODE_STAND, duration=500, pitch=current_pitch, roll=current_roll)
                print("\r>> 恢复正常姿态                  ", end='', flush=True)
